...
function collectEventEntries(configSources: string[]): Array<{ key: string; type: EventType }> {
  ...
  for (const source of configSources) {
    for (const [type, enumType] of [['buff', 'BuffEventId'], ['debuff', 'DebuffEventId'], ['mech', 'MechEventId']] as const) {
      const pattern = new RegExp(`eventCatalogEffectId\\[\\s*(?:(?:EVENT_[A-Z]+_OFFSET|\\d+)\\s*\\+\\s*)?${enumType}\\.([A-Z0-9_]+)\\s*\\]\\s*=`, 'g');
...
function resolveEventMacros(eventKey: string, eventType: string, configSources: string[]) {
  ...
  const titlePattern = new RegExp(
    `eventName\\[\\s*(?:(?:EVENT_[A-Z]+_OFFSET|\\d+)\\s*\\+\\s*)?${enumType}\\.${escapedKey}\\s*\\]\\s*=\\s*STR_EVT_${type}_(\\d+)_TITLE`
  );
  const durationPattern = new RegExp(
    `eventDuration\\[\\s*(?:(?:EVENT_[A-Z]+_OFFSET|\\d+)\\s*\\+\\s*)?${enumType}\\.${escapedKey}\\s*\\]\\s*=\\s*(EVT_[A-Z0-9_]+)`
  );
  const weightPattern = new RegExp(
    `eventCatalogWeight\\[\\s*(?:(?:EVENT_[A-Z]+_OFFSET|\\d+)\\s*\\+\\s*)?${enumType}\\.${escapedKey}\\s*\\]\\s*=\\s*(EVT_[A-Z0-9_]+)`
  );
