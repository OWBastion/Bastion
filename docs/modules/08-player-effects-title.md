...
`masteryRunCode` 在 `env/game.opy` 的全局新局初始化中生成一次，格式为三个 `1000..9999` 数字组。它不随玩家重生、换英雄、重新连接式初始化或玩家进度重置而变化；只有正常新局启动才会生成新值。

HUD 使用 `STR_HUD_RUN_CODE` 在既有左侧结算统计区域显示该值，不调用平台或改变游戏逻辑。`OWBastion/ocrkit` 应把这个字段作为结构化证据处理，并在 Bastion 实际发布后再记录最小兼容游戏/布局版本；本仓库不会预先声明发布版本。
