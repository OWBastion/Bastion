# 附录：`src/` 文件索引

## src/bastion
- `src/bastion/init.opy`

## src/blacklist
- `src/blacklist/init.opy`

## src/config
- `src/config/eventConfigDev.opy`
- `src/config/eventConfig.opy`

## src/constants
- `src/constants/event_constants.opy`
- `src/constants/player_constants.opy`

## src/effects
- `src/effects/player.opy`
- `src/effects/nano.opy`
- `src/effects/init.opy`

## src/env
- `src/env/game.opy`
- `src/env/env_dev.opy`
- `src/env/env.opy`

## src/events
- `src/events/allocation/rejectSampling.opy`
- `src/events/allocation/assignPlayerEvent.opy`
- `src/events/effects/buffEffects.opy`
- `src/events/effects/debuffEffects.opy`
- `src/events/effects/mechEffects.opy`
- `src/events/init/detectFlag.opy`

## src/heroes
- `src/heroes/mercy.opy`
- `src/heroes/zenyatta.opy`
- `src/heroes/all_hero.opy`
- `src/heroes/dva.opy`
- `src/heroes/juno.opy`
- `src/heroes/lifeweaver.opy`
- `src/heroes/reinhardt.opy`
- `src/heroes/ana.opy`
- `src/heroes/symmetra.opy`

## src/locales
- `src/locales/zh-CN.opy`
- `src/locales/en-US.opy`

## src/map
- `src/map/aatlis.opy`
- `src/map/junkertown.opy`
- `src/map/eichenwalde.opy`
- `src/map/route66.opy`
- `src/map/midtown.opy`
- `src/map/busan.opy`
- `src/map/hollywood.opy`
- `src/map/paraiso.opy`
- `src/map/ayutthaya.opy`
- `src/map/lijiang_tower.opy`
- `src/map/kings_row.opy`
- `src/map/dorado.opy`
- `src/map/blizzworld.opy`
- `src/map/havana.opy`
- `src/map/suravasa.opy`
- `src/map/setup_all_map.opy`
- `src/map/throne_of_anubis.opy`
- `src/map/runasapi.opy`
- `src/map/ilios.opy`
- `src/map/numbani.opy`
- `src/map/rialto.opy`
- `src/map/new_queen_street.opy`
- `src/map/oasis.opy`
- `src/map/samoa.opy`
- `src/map/hanamura.opy`
- `src/map/shambali.opy`
- `src/map/paris.opy`
- `src/map/temple_of_anubis.opy`
- `src/map/esperanca.opy`
- `src/map/colosseo.opy`
- `src/map/volskaya.opy`
- `src/map/hanaoka.opy`
- `src/map/circuit_royal.opy`
- `src/map/new_junk_city.opy`
- `src/map/horizon.opy`
- `src/map/nepal.opy`
- `src/map/gibraltar.opy`

## src/player
- `src/player/status.opy`
- `src/player/achievement.opy`
- `src/player/init.opy`

## src/special
- `src/special/setPlayerEvent.opy`
- `src/special/packet.opy`
- `src/special/getRedPacket.opy`
- `src/special/achievement.opy`
- `src/special/rollActivePacket.opy`
- `src/special/locale.opy`
- `src/special/map/lijiang_tower.opy`
- `src/special/event.opy`
- `src/special/eventConfig.opy`

## src/title
- `src/title/title-cn.opy`
- `src/title/init.opy`

## src/tools
- `src/tools/playerNameToIndex.js`
- `src/tools/playerNameToIndexDelimited.js`

## src/utilities
- `src/utilities/updatePlayerStats.opy`
- `src/utilities/playerRegen.opy`
- `src/utilities/setPlayerEvent.opy`
- `src/utilities/setDifficulty.opy`
- `src/utilities/createBastionBot.opy`
- `src/utilities/progressHero.opy`
- `src/utilities/mapDetection.opy`
- `src/utilities/removeFromBlacklist.opy`
- `src/utilities/setPlayerTitle.opy`
- `src/utilities/setEventDuration.opy`
- `src/utilities/setThirdPerson.opy`
- `src/utilities/hashtag.opy`
- `src/utilities/unlockAchievement.opy`
- `src/utilities/devTool.opy`
- `src/utilities/clearEventEffect.opy`
- `src/utilities/healthPool.opy`
- `src/utilities/anticrash.opy`
- `src/utilities/savePlayerData.opy`
- `src/utilities/clearPlayerEvent.opy`
- `src/utilities/resetPlayerCD.opy`
- `src/utilities/setPlayerHP.opy`
- `src/utilities/macros.opy`
- `src/utilities/dashDetector.opy`
- `src/utilities/startCombatRegen.opy`
- `src/utilities/setPlayerSize.opy`
