...
`rejectSampling` 通过 `random.uniform(0, eventWeight) < effectiveWeight(candidate)` 决策是否命中，最多 8 轮；失败时采用最后一次候选，保证有限终止。`effectiveWeight` 在局内经过时间不超过 60 分钟时等于目录基础权重，之后将基础权重向 `eventWeight`（当前由 `EVT_INIT_EVENT_WEIGHT` 初始化）线性压缩：60–120 分钟达到约 15%，120–180 分钟达到约 20%，此后保持 20% 封顶，且不修改目录基础权重。旧的 `42.5 / 37.5 / 20` 类别概率不再作为隐含乘数；全局池的最终逐事件权重平衡属于后续发布前的独立平衡工作。
